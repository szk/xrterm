...
const width = 800;
const height = 600;
describe('WebGL Renderer Integration Tests', function () {
    it('dispose removes renderer canvases', async () => {
        await setupBrowser();
        chai_1.assert.equal(await page.evaluate(`document.querySelectorAll('.xterm canvas').length`), 3);
        await page.evaluate(`addon.dispose()`);
        chai_1.assert.equal(await page.evaluate(`document.querySelectorAll('.xterm canvas').length`), 0);
        await browser.close();
    });
    describe('colors', () => {
...
async function getCellColor(col, row) {
    await page.evaluate(`
...
async function pollFor(page, evalOrFn, val, preFn) {
    if (preFn) {
        await preFn();
    }
